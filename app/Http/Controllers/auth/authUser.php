...
class authUser extends Controller
{
    ...
    public function loginPost(Request $request){
        ...
        if($validate->fails()){
            ...
            $cek = UserValidation::where('email', $request->email)->first();
            if($cek){
                if($cek->active == 1){
    ...
    {
        $datas = UserValidation::where('key', $key)->first();
        if($datas){
            $datas->active = 1;
            $datas->save();
    ...
    public function passwordReset($key){
        $data = UserValidation::where('key', $key)->first();
        if($data){
    ...
    public function passwordResetAction(Request $request){
        ...
        if($validate->fails()){
            ...
            if($data->save()){
                $create_key = $data->email . date('YmdHis');
                $key = md5($create_key);
                UserValidation::where('id_users', $id)->update([
                    'key' => $key
                ]);
